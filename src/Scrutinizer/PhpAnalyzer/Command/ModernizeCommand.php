...
class ModernizeCommand extends Command
{
    protected function configure()
    {
        $this
            ...
            ->addOption('format', 'f', InputOption::VALUE_REQUIRED, 'The file format ("fixture")', 'fixture')
        ;
    }

    protected function execute(InputInterface $input, OutputInterface $output)
    {
        $this->executeFixtureModernization($input->getArgument('dir'));
    }

    protected function executeFixtureModernization($dir)
    {
        uksort(self::$modernizationMap, function($a, $b) {
            return strlen($b) - strlen($a);
        });
        foreach(self::$modernizationMap as $search => $replace) {
            $replace = addslashes($replace);
            ProcessBuilder::create([
                'find', '.', '-type', 'f', '-exec',
                'sed', '-i', "s/$search/$replace/g", '{}', ';'
            ])->setWorkingDirectory($dir)->getProcess()->mustRun();
        }
    }

    protected static $modernizationMap = [
